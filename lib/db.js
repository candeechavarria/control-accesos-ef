...
const DEFAULT_CONFIG = {
  ...
  whatsappSalida: '',
  alertaDatosHoras: 4,
  // Accesos separados de generador y verificador. Apagado: solo se trabaja con el rol mixto.
  rolesSeparados: false,
};
...
async function init() {
  ...
  // Una sola vez: WhatsApp de salida que pidió Cande el 25/09/2026. La marca evita pisar cambios posteriores del admin.
  await run(`UPDATE config SET datos = datos || '{"whatsappSalida": "+54 9 3513 87-7382", "_waInicial": true}'::jsonb
             WHERE id = 1 AND NOT (datos ? '_waInicial')`);
